...
public class Controller implements Initializable {
                    ...
                    while (true) {
                        ...
                        if (str.startsWith("/")) {
                            ...
                            if (str.startsWith("/authok")) {
                                ...
                                loadHistory(100);
                                textArea.appendText("---------------------Новые сообщения------------------\n");
                                break;
    ...
    private void loadHistory(int limitHistory) throws IOException {
        File history = new File("history_" + nickname + ".txt");
        if (!history.exists()) {
            history.createNewFile();
        }
        ...
        if (historyList.size() > limitHistory) {
            for (int i = historyList.size() - limitHistory; i <= (historyList.size() - 1); i++) {
                textArea.appendText(historyList.get(i) + "\n");
            }
        } else if (historyList.size() < limitHistory) {
            for (int i = 1; i <= historyList.size(); i++) {
                textArea.appendText(historyList.get(i - 1) + "\n");
            }
        } else {
